...
begin
	...
	c_matrix = FC_matrix(Ms, [50,150], 0.5)
...
function bar_graph(ks, N, p, x_apex, 
	...
	g1 = blazed_grating_1d(N, p, x_apex; thickness=500, 							mill_depth=mill_depths[1])
	g2 = blazed_grating_1d(N, 2*p, x_apex; thickness=500, 							mill_depth=mill_depths[2])
	g2_shift = -floor(Int, x_apex * ((N÷p)-1) ) - 2
    g2 = circshift(g2, g2_shift)
...
begin
	...
	bar_graph(ks,N,p,x_apex,phase_slider,[100,200],true; ylims=(0,0.63))
end
...
begin
	...
	fig2_F = heatmap(Ms, Ms,(abs2.(diagm(F.S)))
					)
